...
- **Background message types:** `wb:list-conflicts-count` (counts conflict-tagged observations across `observations/*.md` for HUD badge) and `wb:append-conversation-turns` (the passive-log writer).

- `docs/VENDOR_INBOUND.md` — inverse playbook for when major AI vendors reach out about partnership, acquisition, legal concerns, or funding.
- In-extension help menu at `src/help/help.html`. Accessible from the popup footer. Covers what WhiteBox is, how the vault works, how injection/capture/digest flow, the verbatim-only rule, tags, the 5-grade confidence scale, scopes, supported sites, troubleshooting, privacy model.
