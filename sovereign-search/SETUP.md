...
### 4. Run it
- **Windows:** right-click `start-sovereign-search.ps1` (in this folder) → *Run with PowerShell*. It
  starts both servers (logging silenced) and opens the browser.
- **Linux/macOS / manual:** from this `sovereign-search/` folder, in two terminals —
  ```
...
Sovereign Search is **not** exposed to the internet for you. To use it away from your desk, put the
...
```
setx SOVEREIGN_AUTH_TOKEN "<a-long-random-string>"     # Windows (open a new shell after)
export SOVEREIGN_AUTH_TOKEN="<a-long-random-string>"   # Linux/macOS
```
The UI sends it via the `X-Sovereign-Token` header (read from the `#token=` URL fragment, so it
